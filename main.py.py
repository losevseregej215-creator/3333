...
# СОЗДАНИЕ МАГАЗИНА - ЭТИ ФУНКЦИИ ГЛАВНЫЕ!
...
async def create_shop_agreement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if shop_id:
        ...
        await update.message.reply_text(f"✅ Магазин '{shop_name}' создан!", reply_markup=reply_markup)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("add_category_"):
        ...
        category = get_category_products(category_id)
        
        keyboard = [
            [InlineKeyboardButton("➕ Добавить товар", callback_data=f"add_product_{category_id}")],
            [InlineKeyboardButton("🔙 Назад", callback_data=f"back_to_shop")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        text = "Категория\n\n"
        if category:
            for prod_id, name, price, photo, desc in category:
                text += f"• {name} - {price}₽\n"
        else:
            text += "Нет товаров."
    ...
    elif data.startswith("back_to_shop"):
        shop = get_user_shop(user_id)
        if shop:
            shop_id, owner_id, name, about, photo_id, agreement, created_date = shop
            keyboard = [
            ...
            if photo_id:
                await query.edit_message_media(media=photo_id, caption=caption, reply_markup=reply_markup)
            else:
...
def main():
    ...
    # СОЗДАНИЕ МАГАЗИНА - ПРАВИЛЬНЫЙ ConversationHandler
    ...
    category_conv = ConversationHandler(
        ...
        states={1: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_callback)]},
        fallbacks=[CommandHandler("cancel", cancel)]
    )
